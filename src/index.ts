...
const CookieJarSchema = z.string().describe(
  "Naukri cookies. Paste the full cookie string from browser DevTools " +
  "(Application > Cookies > naukri.com). Must include 'nauk_at'."
);

function getCookies(raw: string) {
...
async function getProfileId(cookies: Record<string, string>): Promise<string> {
  const { profileId } = await scrapeProfilePage(cookies);
  if (!profileId) throw new Error("Could not extract profileId from profile page. Check cookies.");
  return profileId;
}
...
server.tool(
  "naukri_get_profile",
  "Get full Naukri profile overview (profileId, name, headline, employment count, skills)",
...
server.tool(
  "naukri_get_profile_id",
  "Extract the profileId from Naukri profile page. Needed for all update operations.",
  { cookies: CookieJarSchema },
  async ({ cookies: raw }) => {
    const cookies = getCookies(raw);
    const { profileId } = await scrapeProfilePage(cookies);
    if (!profileId) return { content: [{ type: "text", text: "Could not extract profileId. Check your cookies." }] };
    return { content: [{ type: "text", text: `Profile ID: ${profileId}` }] };
  }
);

server.tool(
  "naukri_update_summary",
  "Update the profile summary/headline text. Accepts plain text or HTML.",
  {
    cookies: CookieJarSchema,
...
server.tool(
  "naukri_get_ai_suggestions",
  "Get AI-generated profile summary suggestions from Naukri.",
  { cookies: CookieJarSchema },
  async ({ cookies: raw }) => {
    const cookies = getCookies(raw);
...
// ── Employment ───────────────────────────────────────────────────────────
server.tool(
  "naukri_list_employments",
  "List all employments on the Naukri profile with dates and IDs.",
  { cookies: CookieJarSchema },
  async ({ cookies: raw }) => {
    const cookies = getCookies(raw);
...
server.tool(
  "naukri_add_employment",
  "Add a new employment entry to the Naukri profile. Uses the fullprofiles mutation endpoint.",
  {
    cookies: CookieJarSchema,
...
server.tool(
  "naukri_delete_employment",
  "Delete an employment entry by its employmentId (use naukri_list_employments to get IDs).",
  {
    cookies: CookieJarSchema,
...
// ── Skills ───────────────────────────────────────────────────────────────
server.tool(
  "naukri_add_tech_skill",
  "Add or update a tech skill (IT skill) with years/months of experience.",
  {
    cookies: CookieJarSchema,
...
server.tool(
  "naukri_update_key_skills",
  "Update the key skills/tags section of the profile.",
  {
    cookies: CookieJarSchema,
...
// ── Basic Details ────────────────────────────────────────────────────────
server.tool(
  "naukri_update_basic_details",
  "Update professional details: experience, city, salary, notice period, department/industry/role.",
  {
    cookies: CookieJarSchema,
